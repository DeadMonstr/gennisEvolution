...
const RegisterEmployeeStep1 = ({updateUserData,userData}) => {
    ...
    const onSubmit = (data,e) => {
        e.preventDefault()
        updateUserData(data)
        navigate("/registerEmployee/step_2")
    }
    ...
    return (
        <motion.div className="register__step-1">
            <Message activeError={activeError} typeMessage={"error"}>
                {errorMessage}
            </Message>
            <motion.form
                variants={forTitle}
...
export default RegisterEmployeeStep1;